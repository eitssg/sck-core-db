...
class AppActions(RegistryAction):
        ...
        if client_portfolio:
            ...
            if ":" in client_portfolio:
                ...
                raise BadRequestException(
                    'Client-portfolio must be in format "client:portfolio"'
                )
        else:
    ...
    def list(cls, **kwargs) -> Response:
        ...
        try:
            ...
            items = model_class.query(
                hash_key=client_portfolio, attributes_to_get=[APP_KEY]
            )
            result = [a.AppRegex for a in items]
            log.info(
                "Found %d apps for client-portfolio: %s", len(result), client_portfolio
            )
        ...
        try:
            ...
            if "DoesNotExist" in str(type(e)):
                ...
                raise NotFoundException(
                    f"App [{client_portfolio}:{app_regex}] not found"
                )
            else:
                log.error(
                    "Failed to get app %s:%s: %s", client_portfolio, app_regex, str(e)
                )
    ...
    def delete(cls, **kwargs) -> Response:
        ...
        try:
            ...
            if "DoesNotExist" in str(type(e)):
                log.warning(
                    "App not found for deletion: %s:%s", client_portfolio, app_regex
                )
                raise NotFoundException(
        ...
        try:
            ...
            log.error(
                "Failed to delete app %s:%s: %s", client_portfolio, app_regex, str(e)
            )
    ...
    def create(cls, **kwargs) -> Response:
        ...
        try:
            model_class = cls._get_model_class(client)
            log.debug(
                "Creating app: %s:%s with data: %s", client_portfolio, app_regex, kwargs
            )
            ...
            log.error(
                "Invalid app data for %s:%s: %s", client_portfolio, app_regex, str(e)
            )
            raise BadRequestException(f"Invalid app data: {kwargs}: {str(e)}")
        except PutError as e:
            log.warning(
                "App already exists: %s:%s - %s", client_portfolio, app_regex, str(e)
            )
    ...
    def update(cls, **kwargs) -> Response:
        ...
        try:
            ...
            if item:
                log.info(
                    "App exists, will be replaced: %s:%s", client_portfolio, app_regex
                )
        except Exception as e:
            if "DoesNotExist" in str(type(e)):
                log.info(
        ...
        try:
            log.debug(
                "Updating app: %s:%s with data: %s", client_portfolio, app_regex, kwargs
            )
            item: AppFacts = model_class(client_portfolio, app_regex, **kwargs)
            item.save()
            ...
            log.error(
                "Put error updating app %s:%s: %s", client_portfolio, app_regex, str(e)
            )
            raise ConflictException(
                f"App {client_portfolio}:{app_regex} update conflict"
            )
        except Exception as e:
            log.error(
                "Failed to update app %s:%s: %s", client_portfolio, app_regex, str(e)
            )
    ...
    def patch(cls, **kwargs) -> Response:
        ...
        try:
            ...
            new_facts = (
                item.convert_keys(**kwargs) if hasattr(item, "convert_keys") else kwargs
            )
            log.debug(
                "Patching app %s:%s with: %s", client_portfolio, app_regex, new_facts
            )
            ...
            if len(actions) > 0:
                ...
                log.info(
                    "No changes needed for app: %s:%s", client_portfolio, app_regex
                )
            ...
            if "DoesNotExist" in str(type(e)):
                log.warning(
                    "App not found for patch: %s:%s", client_portfolio, app_regex
                )
                raise NotFoundException(
                    f"App {client_portfolio}:{app_regex} does not exist"
                )
                ...
                log.error(
                    "Failed to patch app %s:%s: %s", client_portfolio, app_regex, str(e)
                )
