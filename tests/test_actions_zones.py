...
zone_facts = [
    # Production Zone
    {
        ...
        "account_facts": {
            ...
            "subnet_aliases": ["subnet-prod-public", "subnet-prod-private", "subnet-prod-database"],
            "tags": {
                "Environment": "production",
                "Owner": "platform-team",
            ...
            "us-east-1": {
                ...
                "security_aliases": {
                    "corporate-cidrs": [
                        ...
                        {"type": "CIDR", "value": "172.16.0.0/12", "description": "VPN network range"},
                    ],
                    "admin-cidrs": [{"type": "CIDR", "value": "192.168.1.0/24", "description": "Admin access network"}],
                },
                ...
                "tags": {"Region": "us-east-1", "AZ": "multi-az", "NetworkTier": "production"},
            },
            "us-west-2": {
                ...
                "security_aliases": {
                    "corporate-cidrs": [{"type": "CIDR", "value": "10.0.0.0/8", "description": "Corporate network range"}]
                },
                "security_group_aliases": {"web-sg": "sg-web-prod-west-12345", "app-sg": "sg-app-prod-west-67890"},
        ...
        "tags": {"Environment": "production", "Team": "platform", "Monitoring": "enhanced", "SLA": "tier1"},
    },
    # UAT Zone
    {
        "client": client,
        ...
        "account_facts": {
            ...
            "tags": {"Environment": "uat", "Owner": "qa-team", "CostCenter": "testing", "AutoShutdown": "enabled"},
        },
        "region_facts": {
            "us-central-1": {
                ...
                "security_aliases": {
                    "test-cidrs": [{"type": "CIDR", "value": "172.20.0.0/16", "description": "UAT network range"}],
                    "qa-cidrs": [{"type": "CIDR", "value": "192.168.100.0/24", "description": "QA team access"}],
                },
                "security_group_aliases": {
                ...
                "no_proxy": "*.uat.acme.com,172.20.0.0/16",
                "name_servers": ["8.8.8.8", "1.1.1.1"],
                "tags": {"Region": "us-central-1", "Purpose": "testing", "TestData": "enabled"},
            }
        },
        "tags": {"Environment": "uat", "Team": "qa", "Purpose": "testing", "Temporary": "true"},
    },
    # Development Zone
    {
        "client": client,
        ...
        "account_facts": {
            ...
            "kms": {"aws_account_id": "345678901234", "delegate_aws_account_ids": ["345678901234"], "allow_sns": False},
        ...
        "region_facts": {
            "us-west-2": {
                "aws_region": "us-west-2",
                "az_count": 2,
                "image_aliases": {"ubuntu": "ami-dev123456789", "amazon-linux": "ami-dev987654321", "dev-tools": "ami-devtools123"},
                "min_successful_instances_percent": 25,
                "security_aliases": {
                    "dev-cidrs": [{"type": "CIDR", "value": "172.30.0.0/16", "description": "Development network range"}],
                    "developer-cidrs": [
                        {"type": "CIDR", "value": "192.168.200.0/24", "description": "Developer workstation access"}
                    ],
                },
                "security_group_aliases": {
                ...
                "name_servers": ["8.8.8.8", "1.1.1.1"],
                "tags": {"Region": "us-west-2", "Purpose": "development", "DebugMode": "enabled", "LogLevel": "debug"},
            }
        },
        "tags": {"Environment": "development", "Team": "engineering", "Experimental": "true", "CostOptimized": "true"},
    },
]
...
def test_zone_fact_create(bootstrap_dynamo):
    ...
    for i, zone_fact in enumerate(zone_facts):
        ...
        assert isinstance(response.data, dict), f"Response data is not dict for zone {i+1}"

        # Verify PascalCase keys in response
        assert "Zone" in response.data, f"Zone key missing for zone {i+1}"
        assert response.data["Zone"] == zone_fact["zone"], f"Zone mismatch for zone {i+1}"
        ...
        assert account_facts["AwsAccountId"] == zone_fact["account_facts"]["aws_account_id"]
        ...
        assert "AwsRegion" in region_facts[first_region], f"AwsRegion missing for zone {i+1}"
...
def test_zone_list_with_pagination():
    ...
    if page1.metadata.get("cursor"):
        page2 = ZoneActions.list(client=client, limit=2, cursor=page1.metadata["cursor"])
...
def test_zone_update_full():
    ...
    update_data = {
        ...
        "account_facts": {
            ...
            "tags": {"Environment": "staging", "Owner": "updated-dev-team", "Purpose": "updated-testing"},
        },
        "region_facts": {
            "us-west-2": {
                "aws_region": "us-west-2",
                "az_count": 3,  # Changed from 2
                "image_aliases": {"ubuntu": "ami-updated123456", "new-image": "ami-newimage123"},
        ...
        "tags": {"Environment": "staging", "Team": "updated-engineering", "UpdatedField": "updated-value"},
    }
    ...
    patch_data = {
        ...
        "region_facts": {
            "us-east-1": {
                ...
                "image_aliases": {"ubuntu": "ami-patched-ubuntu", "patched-image": "ami-new-patched"},  # Updated  # Added
...
def test_missing_required_parameters():
    ...
    with pytest.raises(BadRequestException):
        ZoneActions.create(
            client=client, account_facts={"aws_account_id": "123456789012"}, region_facts={"us-test-1": {"aws_region": "us-test-1"}}
        )
...
def test_delete_zone():
    ...
    delete_test_data = {
        ...
        "account_facts": {"aws_account_id": "999999999999", "account_name": "Zone for Deletion Test", "environment": "test"},
...
def test_response_casing_consistency():
    ...
    assert hasattr(get_response, "message") or not hasattr(get_response, "message")  # May not have message

    # Data content should be PascalCase
    data = get_response.data
    expected_pascal_keys = ["Zone", "AccountFacts", "RegionFacts", "Tags", "CreatedAt", "UpdatedAt"]

    for key in expected_pascal_keys:
        if key in ["CreatedAt", "UpdatedAt", "Tags"]:
            continue  # These might be optional
        assert key in data, f"Expected PascalCase key '{key}' not found in response data"
...
def test_audit_fields():
    ...
    patch_response = ZoneActions.patch(client=client, zone=zone_name, tags={"TestPatch": "timestamp-test"})
...
def test_complete_zone_lifecycle():
    ...
    create_data = {
        "client": client,
        "zone": zone_name,
        "account_facts": {"aws_account_id": "111111111111", "account_name": "Lifecycle Test Account", "environment": "test"},
        "region_facts": {
            "us-lifecycle-1": {"aws_region": "us-lifecycle-1", "az_count": 2, "proxy_host": "proxy-lifecycle.test.com"}
        },
    ...
    # 3. Update (partial)
    patch_response = ZoneActions.patch(client=client, zone=zone_name, tags={"Test": "lifecycle", "Stage": "updated"})
    ...
    update_response = ZoneActions.update(
        client=client,
        zone=zone_name,
        account_facts={"aws_account_id": "111111111111", "account_name": "Updated Lifecycle Account", "environment": "staging"},
        region_facts={"us-lifecycle-1": {"aws_region": "us-lifecycle-1", "az_count": 3, "proxy_host": "proxy-updated.test.com"}},
